...
class Car():
    ...
    def __init__(self, device):
        ...
        if USE_TORCH_EFF_MAP:
            self.e_motor = ElMotor_torch(device, net_name = 'Net_10_15_15_5')
        else:
    ...
    def update(self, dt, norm_e_torque, norm_br_torque):
...
class Agent:
    ...
    def update(self, parameters, dt):
        ...
        """
        norm_e_torque, norm_br_torque = parameters
        self._car.update(norm_e_torque=norm_e_torque, norm_br_torque=norm_br_torque, dt=dt)
...
class RobustnessComputer():
    ...
    def compute(self, model, k=-10):
        ...
        rob_power = self.dqs_power.compute(power=torch.cat(power))/self.norm_power

        # print(rob_dist, rob_power, self.alpha*rob_dist+(1-self.alpha)*rob_power)

        return self.alpha*rob_dist+(1-self.alpha)*rob_power
